...
! gfortran -o calcnfehr -Wall -pedantic -std=f95 -fbounds-check -O
! -Wuninitialized -ffpe-trap=invalid,zero,overflow -fbacktrace
! -g calcnfehr.f90 -free -m64
!----------------------------------------------------------------------
module constants
  implicit none
...
contains
  subroutine read_input_nfe
    ...
    return
  end subroutine read_input_nfe
end module common_nfe
!----------------------------------------------------------------------
module allocate_data_nfe
  implicit none
  ! NPT
...
!----------------------------------------------------------------
  ...
  use allocate_data_nfe
  implicit none
  integer(4)::I,J,L,II,JJ,LL,J1,J2,I2,LE,LB,IP
  ...
  !
  write(6,'(A)') '==============calcnfehr v.1.00================'
  !
  write(6,'(A)') 'call read_input_nfe'
  call read_input_nfe
  !
  !bag fix
  TN = 0.0
  ...
  do I=1,NPO
    do L=1,1
      do J=1,MMIN
        TEN=AMA1(I,J)*AFFF(1,I,J)
        TF1=AFFF(1,I,J)
        TFF=TF1**(2.d0)
        do LL=1,LMAX
          do JJ=1,MMIN
            F2=AFFF(LL,I,JJ)
            !if( (AKKK(LL,I,JJ)<=4) .and. (AMA1(I,JJ)>30) ) F2=0.0
            !if( (AKKK(LL,I,JJ)>4)  .and. (AMA1(I,JJ)>(30+(AKKK(LL,I,JJ)-4)*2.5)) ) F2=0.0
            if( AMA1(I,JJ)>(35+AMA1(I,J)+AKKK(1,I,J)*2) ) F2=0.0
            !if( AMA1(I,JJ)>(35+AMA1(I,J)) ) F2=0.0
            if( (LL==1) .and. (JJ==J) ) F2=0.0
            AD=abs( AKKK(LL,I,JJ)-AKKK(1,I,J) )
            if( (AD<0.01) .and. (AFFF(LL,I,JJ)>0.0) ) then
              TEN =TEN + AMA1(I,JJ)*F2
              TF1 =TF1 + F2
              TFF =TFF + F2**(2.d0)
            end if
            if( (JJ==MMIN) .and. (LL==LMAX) ) then
              ! CG and NFE-HR case
              BKKK(I,J) = AKKK(1,I,J)
              if ( TF1 /= 0.0 ) then
                BFFF(I,J) = TFF / TF1
  ...
  !
  write(6,'(A)') '==============WRITE AKK4,5,6.DATA============='
  write(6,'(A)') 'allocate PR,WV'
  allocate(PR(LMAX))
  allocate(WV(LMAX))
  open(4,file='AKK6.DATA')
  open(14,file='AKK7.DATA')
  open(24,file='AKK8.DATA')
  write(4,*)  'EC2cg005, C2cg005'
  write(14,*) 'EC2cg020, C2cg020'
  write(24,*) 'EC2cg050, C2cg050'
  do I=1,NPO
    do LL=1,1
      do J=1,MMIN
        PR(LL)=BFFF(I,J)
  ...
  close(24)
  !
  write(6,'(A)') '========CALC NFE-HR (FDENS+DENS+SORT)========='
  ...
  open(10,file='AKK5.DATA')
  write(10,'(A)') 'ENERGY,NFE,EOA,Ccg2'
  do LE=1,NE
    EE=EM(LE)*EV
...
!      WRITE(10,'(F14.8,6E14.7)') .0,CCP,CCP1,CP,SSS/GEE,CNT(LE)/ML/EV,SNT(LE)
!     write(10,'(A)') 'ENERGY, NFE, Ccg2'
      WRITE(10,'(4(f10.5,1a))') .0,",",CCP,",",CCP1,",",CP," "
    end if
    !
    CC=CNT(LE)
    C2=FNT(LE)
...
!    WRITE(10,'(F14.8,6E14.7)') EE,CCC,CCC1,CF,SSS/GEE,CNT(LE)/ML/EV,SNT(LE)
!     write(10,'(A)') 'ENERGY, NFE, Ccg2'
    WRITE(10,'(4(f10.5,1a))') EE,",",CCC,",",CCC1,",",CF," "
  end do
  CLOSE(10)
  !
