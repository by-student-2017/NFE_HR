...
! file name   :                      f65   f66                     f70
! file type   :                      inp   inp                     out
! file data ----------------------------------------------------------
...
! version dependence -------------------------------
    if (reador(1:28) == 'Time sum iouter   (hamilt, c') then
      if (now_num_of_k_point == 1) then
        read(26, '( 40x, i9 )') read_nlo
        rewind(99)
        write(99, *) read_nlo
        write (6, * ) 'WIEN2k version 14'
        pre_line_nlo_chara = 'Time sum iouter   (hamilt, c'
      endif
    endif
! version dependence -------------------------------
    read(26, '( a130 )') reador
...
!
  do i = 1, ff_array_number
    do j = 1, energy_array_number
      ...
      !if( wave_coef_2(i, j) >= 0.55 .and. eigenvalue_array_Ry(i) <= 0) then
      !  write(6,*) i, j, wave_K2(i), wave_coef_2(i, j), eigenvalue_array_Ry(i)
      !end if
      !
