...
!module ff_cons
  ...
  integer,parameter :: check_file_option  = 3
  !
...
!
! Normalized by axis
  write(6, * ) 'Normalized by axis : a, b, c or n(=No normalization)?'
...
!---- write_K_vs_coefficients ------------------------------------------
  ...
  do i = 1, energy_line_end_num
    if(eigenvalue_array_sub_ef_eV(i) >= (-1 * threshold_energy) ) then
      start_K2_E = i
      exit
    end if
  end do
  do i = 1, energy_line_end_num
    if(eigenvalue_array_sub_ef_eV(i) >= threshold_energy ) then
      end_K2_E = i
      exit
    end if
  end do
!  open (8, file = 'f08')
  filename(1:20)  = 'calcff_K2_vs_energy_'
  filename(21:26) = input_k_point
  filename(27:30) = '.txt'
  open (8, file = filename)
  !
!  write(8, 810) (eigenvalue_array_sub_ef_eV(i), i = 1, energy_line_end_num) 
  write(8, 810) (eigenvalue_array_sub_ef_eV(i), i = start_K2_E, end_K2_E) 
!  810 FORMAT( 'degeneracy fact', ',', '   G^2 / eV    ', ',', 999( 'E=', f13.8, ',') )
  810 FORMAT( '   G^2 / eV    ', ',', 500( 'E=', f13.8, ',') )
...
!  820 FORMAT( i15, ',', f15.8, ',', 999( f15.8, ',' ) ) 
    write(8, 820) wave_K2(i), (wave_coef_2(i, j), j = start_K2_E, end_K2_E)
  820 FORMAT( f15.8, ',', 500( f15.8, ',' ) ) 
  end do 
  !
  close (8)
! check start --------------------------------------------------------  
...
!  open (9, file = 'f09') 
  filename(1:20)  = 'calcff_energy_vs_K2_'
  filename(21:26) = input_k_point
  filename(27:30) = '.txt'
